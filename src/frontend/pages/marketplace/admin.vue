<template>
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="flex justify-between items-center py-6">
          <div>
            ...
            <p class="mt-2 text-gray-600">
              Manage marketplace operations and debug issues
            </p>
          </div>
          <UBadge color="red" variant="soft" size="lg"> Admin Only </UBadge>
        </div>
      </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <!-- All Escrows -->
      <div
        v-if="allEscrows.length > 0"
        class="bg-white rounded-lg shadow-sm border mb-8"
      >
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="[askId, escrow] in allEscrows" :key="askId.toString()">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {{
                    escrow.buyer?.[0]
                      ? formatPrincipal(escrow.buyer[0].owner.toString())
                      : 'N/A'
                  }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
      ...
      <!-- Retry Info -->
      <div
        v-if="retryInfo.length > 0"
        class="bg-white rounded-lg shadow-sm border mb-8"
      >
        <div class="p-6 border-b">
          <h3 class="text-lg font-semibold text-gray-900">
            Settlements Needing Retry ({{ retryInfo.length }})
          </h3>
        </div>
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
...
<script setup lang="ts">
  ...
  const loadRetryInfo = async () => {
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to load retry info'
    } finally {
  ...
  const handleRetrySettlement = async (askId: bigint) => {
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to retry settlement'
    } finally {
