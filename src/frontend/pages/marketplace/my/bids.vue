      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-6">
          <div>
            ...
            <p class="mt-2 text-gray-600">Track your bids and purchases on the marketplace</p>
          </div>
          <UButton
            to="/marketplace"
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-lg shadow-sm border p-6">
          <div class="flex items-center">
            <div class="p-2 bg-blue-100 rounded-lg">
              <UIcon name="i-heroicons-hand-raised" class="w-6 h-6 text-blue-600" />
            </div>
            <div class="ml-4">
              <div class="text-2xl font-bold text-gray-900">{{ userActiveBids.length }}</div>
        ...
        <div class="bg-white rounded-lg shadow-sm border p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <div class="text-2xl font-bold text-gray-900">{{ successfulBids.length }}</div>
        ...
        <div class="bg-white rounded-lg shadow-sm border p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <div class="text-2xl font-bold text-gray-900">{{ outbidBids.length }}</div>
        ...
        <div class="bg-white rounded-lg shadow-sm border p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <div class="text-2xl font-bold text-gray-900">{{ totalBidAmount }}</div>
      ...
      <div class="bg-white rounded-lg shadow-sm border">
        ...
        <div class="p-6">
          ...
          <div v-else-if="userBidsError" class="text-center py-12">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2">Failed to load bids</h3>
            <p class="text-gray-600 mb-4">{{ userBidsError }}</p>
            <UButton @click="loadUserBids" variant="outline">
              Try Again
            </UButton>
          </div>
          ...
          <div v-else-if="getTabBids().length === 0" class="text-center py-12">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2">No {{ getTabName() }} bids</h3>
                ...
                <div class="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-gray-500">
                      {{ getStatusText(bid.status) }} • {{ bid.participants.length }} participants
                    </p>
                    <p class="text-sm text-gray-500">
...
const successfulBids = computed(() => {
  return userBids.value.filter(bid => 
    getAskStatusText(bid.status) === 'Closed' && 
    bid.settled_at && 
    bid.settled_at[0][0].toString() === authStore.identity?.getPrincipal().toString()
  )
})
...
const outbidBids = computed(() => {
  return userBids.value.filter(bid => 
    getAskStatusText(bid.status) === 'Closed' && 
    (!bid.settled_at || bid.settled_at[0][0].toString() !== authStore.identity?.getPrincipal().toString())
  )
})

const totalBidAmount = computed(() => {
  return userBids.value.reduce((total, bid) => {
...
const canClaim = (bid: AskStatus) => {
  return getStatusText(bid.status) === 'Closed' && 
         bid.settled_at && 
         bid.settled_at[0][0].toString() === authStore.identity?.getPrincipal().toString()
}
