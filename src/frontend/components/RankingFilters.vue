<template>
  <div class="flex items-center gap-2 w-full">
    ...
    <div class="relative" ref="dropdownRef">
      <button @click="showDropdown = !showDropdown" class="flex items-center gap-1 px-3 h-8 rounded-md border text-xs font-semibold transition bg-white dark:bg-neutral-950 border-gray-200 dark:border-gray-700 hover:bg-primary-50 dark:hover:bg-primary-500">
        <span>{{ selectedTime }}</span>
        <UIcon name="mdi:chevron-down" class="text-base" />
      </button>
      <div v-if="showDropdown" class="absolute right-0 mt-1 w-24 bg-white dark:bg-neutral-950 border border-gray-200 dark:border-gray-700 rounded shadow z-10">
        <button
          ...
          :class="selectedTime === t ? 'text-primary-600' : 'text-gray-700 dark:text-gray-200'"
        >
          {{ t }}
        </button>
      </div>
    ...
    <UTooltip v-if="sidebarStore.isStatsPanelVisible" text="Collapse stats panel" position="bottom">
      <button 
      ...
      >
        <UIcon name="tabler:layout-sidebar-right-collapse-filled" class="text-xl" />
      </button>
    </UTooltip>
  </div>
</template>
