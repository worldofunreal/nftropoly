        ...
        <div class="relative">
          ...
          <div class="relative h-32 bg-gray-100 dark:bg-gray-900 rounded-lg overflow-hidden">
            ...
            <button
              ...
              @click="triggerBannerUpload"
            >
              <UIcon name="i-heroicons-camera-20-solid" class="w-4 h-4 text-white" />
            </button>
          </div>
          ...
          <div class="absolute -bottom-8 left-4">
            <div class="relative">
              ...
              <button
                ...
                @click="triggerAvatarUpload"
              >
                <UIcon name="i-heroicons-camera-20-solid" class="w-3 h-3 text-white" />
              </button>
            </div>
          </div>
        </div>
...
<script setup lang="ts">
  ...
  const handleAvatarFileSelect = async (event: Event) => {
    ...
    if (!auth.canisterInitialized) {
      handleUploadError('Canister service not initialized. Please try logging in again.')
      return
    }
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      const update = {
        display_name: form.value.displayName ? [form.value.displayName] as [string] : [] as [],
        bio: form.value.bio ? [form.value.bio] as [string] : [] as [],
        location: form.value.location ? [form.value.location] as [string] : [] as [],
        website: form.value.website ? [form.value.website] as [string] : [] as [],
        avatar_url: form.value.avatarUrl ? [form.value.avatarUrl] as [string] : [] as [],
        banner_url: form.value.bannerUrl ? [form.value.bannerUrl] as [string] : [] as [],
