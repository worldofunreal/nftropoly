<template>
  ...
  >
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-6">
      ...
      <div class="space-y-3">
        ...
        <div class="flex gap-1">
          <button
            @click="type = 'NFTs'"
            :class="type === 'NFTs' ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200'"
          ...
          <button
            @click="type = 'Tokens'"
            :class="type === 'Tokens' ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200'"
      ...
      <div class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            :class="selectedFilter === filter ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200'"
      ...
      <div class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            :class="selectedTime === time ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200'"
            class="px-3 py-2 rounded font-medium text-sm"
          >
            {{ time }}
          </button>
      ...
      <div class="space-y-3">
        <h4 class="font-medium text-gray-900 dark:text-white">Search for Chains</h4>
        <div class="relative">
          <input
            type="text"
      ...
      <div class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            :class="selectedChains.includes(chain) ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200'"
      ...
      <div class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            :class="selectedMarketCaps.includes(cap) ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200'"
            class="px-3 py-2 rounded-full border text-sm font-medium"
          >
            {{ cap }}
          </button>
      ...
      <div class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            :class="selectedCategoryOptions[cat.label]?.includes(cat.label) ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200'"
            class="px-3 py-2 rounded-full border text-sm font-medium"
          >
            {{ cat.label }}
          </button>
        ...
        <div class="space-y-3">
          <div class="flex items-center gap-3">
            <input
              ...
              v-model="hasNft"
            />
            <label for="mobile-has-nft" class="text-gray-700 dark:text-gray-200 font-medium">Has NFT</label>
          </div>
          <div class="flex items-center gap-3">
            <input
              type="checkbox"
            ...
            />
            <label for="mobile-branded-token" class="text-gray-700 dark:text-gray-200 font-medium">Branded Token Page</label>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Action Buttons -->
    <div class="flex gap-3 p-4 border-t border-gray-200 dark:border-gray-800 flex-shrink-0">
      <button
        @click="resetFilters"
...
const chains = [
  "Ethereum", "Solana", "Arbitrum", "Base", "Optimism", "Polygon"
]
const selectedChains = ref<string[]>([])

const marketCaps = [
  "<$100K", "$100K - $500K", "$500K - $1M", "$1M - $10M", "$10M+"
]
...
const categories = [
  ...
  { label: "Layer 2", open: false, options: ["Arbitrum", "Optimism", "Base"] },
  { label: "Stablecoins", open: false, options: ["USDT", "USDC", "DAI"] },
  { label: "Smart Contract Platform", open: false, options: ["Ethereum", "Solana", "Polygon"] },
...
function toggleCategoryOption(label: string, opt: string) {
  ...
  if (selectedCategoryOptions.value[label].includes(opt)) {
    selectedCategoryOptions.value[label] = selectedCategoryOptions.value[label].filter((o) => o !== opt)
  } else {
