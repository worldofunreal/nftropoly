  ...
  <div class="w-[220px] max-w-[260px] p-4 bg-white dark:bg-gray-950 rounded-lg border border-gray-200 dark:border-gray-800">
    ...
    >
      <div class="flex flex-wrap gap-2 mt-2">
        <button v-for="cap in marketCaps" :key="cap" class="px-3 py-1 rounded-full border text-xs font-semibold bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-200 border-gray-200 dark:border-gray-700">{{ cap }}</button>
      </div>
    </SidebarSection>
    <SidebarSection
      title="Category"
    ...
    >
      <div class="flex flex-col gap-2 mt-2">
        <label class="flex items-center gap-2">
          <input type="checkbox" v-model="hasNFT" class="form-checkbox rounded" /> Has NFT
        </label>
        <label class="flex items-center gap-2">
          <input type="checkbox" v-model="brandedTokenPage" class="form-checkbox rounded" /> Branded Token Page
        </label>
      </div>
    </SidebarSection>
  </div>
...
const openSections = reactive({ chains: true, marketCap: false, category: false })
const chains = [
...
const brandedTokenPage = ref(false)
</script> 