<template>
  <div class="h-full mb-11 w-full bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-950">
    <div v-if="galleries.length === 0" class="flex flex-col items-center justify-center h-full w-full">
      <div class="mb-4">
        <img src="https://undraw.co/api/illustrations/undraw_add_to_cart_re_wrdo.svg" alt="Create Gallery" class="w-32 h-32" />
      </div>
      <div class="text-2xl font-bold mb-2">No Galleries Yet</div>
      <div class="text-gray-400 mb-4 text-center max-w-md">
        Showcase your favorite NFTs with our new galleries section. Create custom collections to share and organize your NFTs. This tab is publicly hidden until you add sections.
      </div>
      <button class="px-6 py-2 rounded bg-primary-600 hover:bg-primary-700 transition text-white font-semibold mb-2">Create a gallery</button>
      <a href="#" class="text-primary-600 hover:underline text-sm">Learn more about galleries</a>
    </div>
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 w-full p-6">
      <div
      ...
      >
        ...
        <div class="font-bold text-lg mb-1 flex items-center gap-2">
          {{ gallery.name }}
          <span v-if="gallery.featured" class="ml-1 px-2 py-0.5 text-xs bg-yellow-200 text-yellow-800 rounded">Featured</span>
        </div>
        <div class="text-gray-500 dark:text-gray-400 text-sm mb-2 line-clamp-2">{{ gallery.description }}</div>
...
const galleries = ref<Gallery[]>([
  {
    ...
    description: 'A curated set of my favorite CryptoPunks NFTs. Showcasing rare traits and unique styles.',
  ...
  },
  {
    id: 2,
    name: 'Art Blocks',
    description: 'Generative art masterpieces from the Art Blocks platform. Vibrant, algorithmic, and inspiring.',
