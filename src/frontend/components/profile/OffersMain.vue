<template>
  <div class="flex flex-col gap-4 p-4 w-full">
    ...
    <div v-if="offers.length === 0" class="flex flex-col items-center justify-center flex-1 py-16">
      ...
      <div class="text-gray-400 mb-4">You currently don't have any active or unfunded offers</div>
      <button class="px-4 py-2 rounded bg-primary-600 text-white font-semibold">View Trending Collections</button>
    </div>
    <div v-else class="overflow-x-auto flex-1">
      <table class="min-w-5xl w-full text-sm">
        <thead>
          <tr class="bg-neutral-100 dark:bg-neutral-800">
            <th class="p-4 text-left font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Offer</th>
            <th class="p-4 text-left font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Status</th>
            <th class="p-4 text-right font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Price</th>
            <th class="p-4 text-right font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Top Offer</th>
            <th class="p-4 text-right font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Floor</th>
            <th class="p-4 text-right font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Qty</th>
            <th class="p-4 text-right font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Total</th>
            <th class="p-4 text-right font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Exp</th>
            <th class="p-4 text-right font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Time</th>
          </tr>
        </thead>
        <tr class="border-b border-gray-200 dark:border-gray-700 h-px"></tr>
        <tbody>
          <tr v-for="offer in offers" :key="offer.id" class="border-b border-gray-200 dark:border-gray-800 hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
            <td class="p-4">
              <div class="flex flex-col">
                <span class="font-semibold text-gray-900 dark:text-white">{{ offer.name }}</span>
                <span class="text-sm text-gray-600 dark:text-gray-400">#{{ offer.id }}</span>
              </div>
            </td>
            <td class="p-4 text-left whitespace-nowrap">
              <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium" 
                    :class="{
                      'bg-green-100 dark:bg-green-900/20 text-green-600 dark:text-green-400': offer.status === 'Active',
                      'bg-yellow-100 dark:bg-yellow-900/20 text-yellow-600 dark:text-yellow-400': offer.status === 'Unfunded'
                    }">
                {{ offer.status }}
              </span>
            </td>
            <td class="p-4 text-right whitespace-nowrap">
              <span class="font-medium text-gray-900 dark:text-white">{{ offer.price }}</span>
            </td>
            <td class="p-4 text-right whitespace-nowrap">
              <span class="text-green-600 dark:text-green-400">{{ offer.topOffer }}</span>
            </td>
            <td class="p-4 text-right whitespace-nowrap">
              <span class="text-gray-600 dark:text-gray-300">{{ offer.floor }}</span>
            </td>
            <td class="p-4 text-right whitespace-nowrap">
              <span class="text-gray-600 dark:text-gray-300">{{ offer.qty }}</span>
            </td>
            <td class="p-4 text-right whitespace-nowrap">
              <span class="font-medium text-gray-900 dark:text-white">{{ offer.total }}</span>
            </td>
            <td class="p-4 text-right whitespace-nowrap">
              <span class="text-gray-600 dark:text-gray-400 text-sm">{{ offer.exp }}</span>
            </td>
            <td class="p-4 text-right whitespace-nowrap">
              <span class="text-gray-600 dark:text-gray-400 text-sm">{{ offer.time }}</span>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                :class="selectedStatus === status ? 'bg-primary-600 text-white border-primary-600' : 'bg-white dark:bg-neutral-950 text-gray-700 dark:text-gray-200 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800'"
          ...
          <div>
            ...
            <select class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-neutral-950">
              <option>Most recent</option>
              <option>Price: Low to High</option>
              <option>Price: High to Low</option>
          ...
          <div>
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-3">
                ...
                <span class="text-sm text-gray-700 dark:text-gray-300">Active Offers</span>
              </label>
              <label class="flex items-center gap-3">
                <input type="checkbox" class="rounded border-gray-300 dark:border-gray-600" />
                <span class="text-sm text-gray-700 dark:text-gray-300">Unfunded Offers</span>
              </label>
              <label class="flex items-center gap-3">
                <input type="checkbox" class="rounded border-gray-300 dark:border-gray-600" />
                <span class="text-sm text-gray-700 dark:text-gray-300">Expired Offers</span>
              </label>
            </div>
          </div>
        </div>
...
const chains = [
  { label: 'All', icon: 'logos:ethereum', color: 'bg-neutral-200 dark:bg-neutral-800' },
