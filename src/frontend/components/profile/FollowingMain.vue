...
<script setup lang="ts">
  ...
  // Unfollow user
  const unfollowUser = async (user: unknown) => {
    if (!user || typeof user !== 'object' || !('id' in user)) return
    const userObj = user as { id: { toText: () => string } }
    unfollowingUser.value = userObj.id.toText()
    try {
      await canisterService.unfollowUser(userObj.id.toText())
