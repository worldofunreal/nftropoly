    ...
    <table class="min-w-full text-sm">
      ...
      <tbody>
        <tr v-for="token in sortedTokens" :key="token.symbol" class="border-b border-gray-200 dark:border-gray-800 hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
          ...
          <!-- 1D VOL -->
          <td class="p-4 font-mono text-gray-600 dark:text-gray-300">{{ token.vol1d }}</td>
          <!-- MARKET CAP -->
          <td class="p-4 font-mono text-gray-600 dark:text-gray-300">{{ token.marketCap }}</td>
          <!-- SUPPLY -->
          <td class="p-4 font-mono text-gray-600 dark:text-gray-300">{{ token.supply }}</td>
          <!-- FDV -->
          <td class="p-4 font-mono text-gray-600 dark:text-gray-300">{{ token.fdv }}</td>
          <!-- LAST 7D (Sparkline) -->
