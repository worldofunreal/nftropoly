...
<script setup lang="ts">
  ...
  async function loginWithMetaMask() {
    ...
    try {
      ...
      const loginResult = await auth.handleLoginFlow(
        finalSeedPhrase,
        ethAddress,
        'metamask'
      )
      ...
      if (loginResult.existing) {
        ...
        console.log(
          'showRegistrationModal set to:',
          showRegistrationModal.value
        )
        await nextTick()
        console.log(
          'After nextTick, registrationModalRef.value:',
          registrationModalRef.value
        )
        if (registrationModalRef.value) {
          console.log(
            'Calling registrationModalRef.value.open with:',
            ethAddress,
            icpPrincipal,
            'metamask'
          )
  ...
  async function loginWithPlug() {
    ...
    try {
      ...
      if (!window.ic || !window.ic.plug) {
        throw new Error(
          'Plug Wallet is not installed. Please install the Plug extension.'
        )
      }
