  ...
  >
    <div class="flex justify-between items-center h-14 mx-4 md:mx-4">
      ...
      <div class="flex items-center gap-2 flex-shrink-0">
        ...
        <!-- Search Bar -->
        <div
          class="hidden md:flex items-center ml-2 search-nfts-section relative"
        >
          <UInput
            v-model="search"
...
<script setup lang="ts">
  ...
  const _followingUser = ref<string | null>(null)
  ...
  const handleFollow = (user: unknown) => {
    // Update the user's following status in search results
    const userObj = user as { id: string; username: string }
    const userIndex = searchResults.value.findIndex(
      (u: unknown) => (u as { id: string }).id === userObj.id
    )
    if (userIndex !== -1) {
      ;(
        searchResults.value[userIndex] as { am_following_them: boolean }
  ...
  const handleUnfollow = (user: unknown) => {
    // Update the user's following status in search results
    const userObj = user as { id: string; username: string }
    const userIndex = searchResults.value.findIndex(
      (u: unknown) => (u as { id: string }).id === userObj.id
