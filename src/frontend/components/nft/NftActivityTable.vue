<template>
  <div class="bg-neutral-800 rounded-lg">
      ...
      <table class="w-full min-w-max">
        ...
        <thead class="bg-neutral-800">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">FROM</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">TO</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">TIME</th>
          ...
          >
            ...
            <td class="px-4 py-3 whitespace-nowrap">
              <div class="flex items-center gap-2">
                <div :class="getEventIcon(activity.event)" class="w-5 h-5 flex-shrink-0"></div>
            ...
            <td class="px-4 py-3 whitespace-nowrap">
              <div v-if="activity.price" class="flex items-center gap-2">
                <img src="https://placehold.co/16x16" alt="ETH" class="w-4 h-4 rounded-full flex-shrink-0" />
            ...
            <td class="px-4 py-3 whitespace-nowrap">
              <div class="flex items-center gap-2">
                <img :src="activity.from.avatar" :alt="activity.from.name" class="w-6 h-6 rounded-full flex-shrink-0" />
                <span class="text-sm font-medium">{{ activity.from.name }}</span>
                <UIcon v-if="activity.from.verified" name="material-symbols:verified" class="w-4 h-4 text-primary-500 flex-shrink-0" />
              </div>
            </td>
            ...
            <td class="px-4 py-3 whitespace-nowrap">
              <div v-if="activity.to" class="flex items-center gap-2">
                <img :src="activity.to.avatar" :alt="activity.to.name" class="w-6 h-6 rounded-full flex-shrink-0" />
                <span class="text-sm font-medium">{{ activity.to.name }}</span>
                <UIcon v-if="activity.to.verified" name="material-symbols:verified" class="w-4 h-4 text-primary-500 flex-shrink-0" />
              </div>
              <span v-else class="text-sm text-gray-400">-</span>
            </td>
    ...
    <div v-if="filteredActivities.length === 0" class="p-8 text-center">
      <UIcon name="i-heroicons-clock-20-solid" class="w-12 h-12 text-gray-500 mx-auto mb-4" />
      <h3 class="text-lg font-semibold text-gray-300 mb-2">No activity yet</h3>
      <p class="text-gray-400">Activity will appear here once transactions occur</p>
    </div>
  </div>
</template>
