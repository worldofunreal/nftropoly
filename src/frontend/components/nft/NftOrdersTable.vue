<template>
  <div class="bg-neutral-800 rounded-lg">
    ...
    <div class="p-4 border-b border-gray-700">
      ...
      <p class="text-sm text-gray-400 mt-1">Active offers for this collection</p>
    </div>
      ...
      <table class="w-full">
        ...
        <thead class="bg-neutral-800">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">FROM</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">EXPIRY</th>
          ...
          >
            ...
            <td class="px-4 py-3">
              <div class="flex items-center gap-2">
                <div :class="getTypeColor(offer.type)" class="w-2 h-2 rounded-full"></div>
            ...
            <td class="px-4 py-3">
              <div class="flex items-center gap-2">
                <img src="https://placehold.co/16x16" alt="WETH" class="w-4 h-4 rounded-full" />
            ...
            <td class="px-4 py-3">
              <div class="flex items-center gap-2">
                <img :src="offer.from.avatar" :alt="offer.from.name" class="w-6 h-6 rounded-full" />
                <span class="text-sm font-medium">{{ offer.from.name }}</span>
                <UIcon v-if="offer.from.verified" name="material-symbols:verified" class="w-4 h-4 text-primary-500" />
              </div>
            </td>
            ...
            <!-- Action -->
            <td class="px-4 py-3">
              <button class="px-3 py-1 bg-primary-500 hover:bg-primary-600 rounded text-sm font-medium transition-colors">
                Accept
              </button>
            </td>
          </tr>
    ...
    <div v-if="offers.length === 0" class="p-8 text-center">
      <UIcon name="i-heroicons-inbox-20-solid" class="w-12 h-12 text-gray-500 mx-auto mb-4" />
      <h3 class="text-lg font-semibold text-gray-300 mb-2">No offers yet</h3>
      <p class="text-gray-400">Be the first to make an offer on this collection</p>
    </div>
  </div>
</template>
