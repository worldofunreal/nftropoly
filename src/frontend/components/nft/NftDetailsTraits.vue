  ...
  <div class="bg-neutral-800 rounded-lg">
    ...
    <div class="flex items-center justify-between p-4 border-b border-gray-700">
      ...
      <div class="flex gap-1">
        <button @click="viewMode = 'grid'" :class="viewMode === 'grid' ? 'text-primary-500' : 'text-gray-400'" class="p-1">
          <UIcon name="i-heroicons-squares-2x2-20-solid" />
        </button>
        <button @click="viewMode = 'list'" :class="viewMode === 'list' ? 'text-primary-500' : 'text-gray-400'" class="p-1">
    ...
    <div v-if="isExpanded" class="p-4">
      <div :class="viewMode === 'grid' ? 'grid grid-cols-2 md:grid-cols-3 gap-3' : 'space-y-2'">
        <div 
          v-for="trait in traits" 
          :key="`${trait.category}-${trait.value}`"
          :class="[
            'p-3 rounded border cursor-pointer transition-colors',
            viewMode === 'grid' ? 'text-center' : 'flex justify-between items-center',
            trait.isRare ? 'border-purple-500 bg-purple-900/20' : 'border-gray-700 hover:border-gray-600'
          ]"
        >
          <div :class="viewMode === 'grid' ? 'space-y-1' : 'flex items-center gap-3'">
            <div class="text-xs text-gray-400 uppercase">{{ trait.category }}</div>
            <div class="font-semibold">{{ trait.value }}</div>
          </div>
          <div :class="viewMode === 'grid' ? 'text-xs text-gray-400' : 'text-right'">
