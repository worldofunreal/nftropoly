  ...
  <div>
    ...
    <div ref="scrollRef" class="flex gap-4 overflow-x-auto scrollbar-hide pb-2" style="scroll-snap-type: x mandatory;">
      <div
        v-for="col in movers"
        :key="col.id"
        class="min-w-[220px] max-w-xs bg-white dark:bg-gray-900 rounded-xl shadow p-3 flex flex-col gap-2 cursor-pointer hover:scale-105 transition-transform relative"
      ...
      >
        ...
        <div class="flex items-center gap-2 mt-1">
          <span :class="col.change >= 0 ? 'text-green-600' : 'text-red-600'" class="text-lg font-extrabold">{{ col.change >= 0 ? '+' : '' }}{{ col.change }}%</span>
        </div>
      </div>
    </div>
  </div>
