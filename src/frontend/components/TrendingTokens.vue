    ...
    <div class="relative">
      ...
      >
        ...
        <div class="flex items-center gap-2 text-sm">
          ...
          <span :class="token.change >= 0 ? 'text-green-600' : 'text-red-600'">{{ token.change >= 0 ? '+' : '' }}{{ token.change }}%</span>
        </div>
        <!-- Line Chart -->
        <div class="relative w-full" style="height:32px; min-height:32px; max-height:32px;">
          <Line :data="token.chartData" :options="chartOptions" style="height:32px; min-height:32px; max-height:32px;" />
        </div>
      </div>
    </div>
  </div>
