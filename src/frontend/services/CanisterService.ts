...
const databaseIdlFactory = ({ IDL }: any) => {
  ...
  return IDL.Service({
    ...
    'getVerifiedUsers' : IDL.Func([IDL.Nat], [IDL.Vec(UserSearchResult)], ['query']),
    'isUsernameAvailable' : IDL.Func([IDL.Text], [IDL.Bool], ['query']),
    'registerUser' : IDL.Func([RegistrationData], [ApiResult], []),
    'searchUsers' : IDL.Func([IDL.Text, IDL.Nat], [IDL.Vec(UserSearchResult)], ['query']),
    'signup' : IDL.Func([IDL.Text], [Result], []),
    'updatePortfolioStats' : IDL.Func([IDL.Float64, IDL.Float64, IDL.Nat, IDL.Nat], ['query']),
    'updateProfile' : IDL.Func([IDL.Record({ 'bio' : IDL.Opt(IDL.Text), 'displayName' : IDL.Opt(IDL.Text), 'socialLinks' : IDL.Opt(SocialLinks), 'email' : IDL.Opt(IDL.Text), 'privacy' : IDL.Opt(PrivacySettings), 'avatarUrl' : IDL.Opt(IDL.Text), 'bannerUrl' : IDL.Opt(IDL.Text), 'avatarPreset' : IDL.Opt(IDL.Nat), 'location' : IDL.Opt(IDL.Text) })], [ApiResult], []),
  });
};
  ...
  async initialize(identity?: Identity) {
    try {
      ...
      this.databaseActor = Actor.createActor(databaseIdlFactory, {
        agent: this.agent,
        canisterId: DATABASE_CANISTER_ID,
      });
