...
export const useApprovalsStore = defineStore('approvals', {
  ...
  getters: {
    ...
    getAllNFTApprovals: (state) => (userKey: string) => {
    ...
    isMarketplaceApprovedForNFT: (state) => (tokenId: string, marketplaceCanisterId: string) => {
      const approvals = state.nftApprovals.get(tokenId) || []
      return approvals.some(approval => 
        approval.spender === marketplaceCanisterId && 
        (!approval.expiresAt || approval.expiresAt > BigInt(Date.now() * 1000000))
      )
    },
    
    // Check if marketplace is approved for tokens
    isMarketplaceApprovedForTokens: (state) => (userKey: string, marketplaceCanisterId: string, amount: bigint) => {
      const approvals = state.tokenApprovals.get(userKey) || []
      const approval = approvals.find(a => a.spender === marketplaceCanisterId)
      
      if (!approval) return false
      if (approval.expiresAt && approval.expiresAt <= BigInt(Date.now() * 1000000)) {
        return false
      }
...
export const createUserKey = (principal: string, subaccount?: Uint8Array): string => {
  return subaccount ? `${principal}-${Array.from(subaccount).join('')}` : principal
}
