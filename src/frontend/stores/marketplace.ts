...
export const useMarketplaceStore = defineStore('marketplace', {
  ...
  getters: {
    ...
    filteredAsks: (state) => {
      ...
      if (state.filters.status.length > 0) {
        filtered = filtered.filter(ask => 
          state.filters.status.includes(ask.status ? Object.keys(ask.status)[0] : '')
        )
      }
    ...
    userActiveAsks: (state) => {
      return state.userAsks.filter(ask => 
        ask.status && 'Open' in ask.status
      )
    },
    ...
    userClosedAsks: (state) => {
      return state.userAsks.filter(ask => 
        ask.status && 'Closed' in ask.status
      )
    },
    
    // User's active bids
    userActiveBids: (state) => {
      return state.userBids.filter(ask => 
        ask.status && 'Open' in ask.status
      )
    },
    ...
    getAskById: (state) => (askId: string) => {
      return state.askDetails.get(askId) || 
...
function getSortValue(ask: AskStatus, sortBy: string): any {
  switch (sortBy) {
    case 'price':
