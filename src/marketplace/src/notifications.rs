...
impl NotificationManager {
    ...
    pub fn subscribe(
        &mut self,
        principal: Principal,
        event_types: Vec<NotificationType>,
    ) -> MarketplaceResult<()> {
    ...
    pub fn notify_bid_placed(
        ...
        let message = format!(
            "Bid placed on ask {} by {} for amount {}",
            ask_id, bidder, amount
        );
    ...
    pub fn notify_kyc_required(
        ...
        let message = format!(
            "KYC verification required for user {} on ask {}",
            user, ask_id
        );
