...
impl AMMManager {
    ...
    pub fn swap(
        ...
        let (reserve_in, reserve_out) = if token_in.canister == pool.token_1.canister
            && token_in.symbol == pool.token_1.symbol
        {
            (pool.token_1_reserve, pool.token_2_reserve)
        } else if token_in.canister == pool.token_2.canister
            && token_in.symbol == pool.token_2.symbol
        {
    ...
    pub fn calculate_price_impact(
        ...
        let new_reserve_out =
            reserve_out - (amount_in_with_fee * reserve_out) / (reserve_in + amount_in_with_fee);
